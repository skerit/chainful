...
Blockchain.setMethod(function verifyProposedBlock(block, callback) {
	...
	pledge = Fn.series(function verifySignatures(next) {
		...
		block.calculateHash(function gotHash(err, hash) {
			...
			hash_string = hash.toString('hex');

			// Check the difficulty
			for (i = 0; i < that.chainful.difficulty; i ++) {
				// As soon as one of the bytes is not 0, it's wrong
				if (hash_string[i] !== '0') {
					return next(new Error('The block hash "' + hash.toString('hex') + '" does not match the difficulty of ' + that.chainful.difficulty));
				}
			}
		...
		// execute that now
		if (that.chainful.block_verifier) {
			that.chainful.block_verifier(block, next);
		} else {
			next();
		}
	}, function done(err) {
