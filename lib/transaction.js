...
Transaction.setMethod(function isOurMinerTransaction(block, public_key) {
	...
	// The data needs to be an array of length 1
	if (!this.data || !Array.isArray(this.data) || !this.data[0] || this.data.length !== 1) {
		return false;
	}

	// The data needs to be the timestamp of the block
	if (this.data[0] !== block.timestamp) {
		return false;
	}
