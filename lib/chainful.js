...
Chainful.setMethod(function requestBlocks(block_indexes, callback) {
	...
	for (i = block_indexes.length; i > 0; i--) {
	...
	}, function proposeBlocks(next) {
		that.proposeBlocks(new_blocks, next);
	}, function done(err) {
...
Chainful.setMethod(function proposeBlocks(input_blocks, callback) {
	...
	Fn.series(function convertBuffers(next) {

		var tasks = [];

		input_blocks.forEach(function eachBlock(block, index) {
		...
		result_blocks.forEach(function eachBlock(block) {
