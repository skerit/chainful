...
Chainful.setMethod(function minePendingTransactions(private_key, public_key, callback) {
	...
	if (!this.last_block) {
		return Blast.nextTick(pledge.reject, pledge, new Error('Can not mine pending transactions when there is no genesis block available'));
	}
...
Chainful.setMethod(function storeChain(target, callback) {
	...
	if (!this.chain.length) {
		return Blast.nextTick(callback, null, new Error('Can not save an empty chain'));
	}
...
Chainful.setMethod(function loadFromBuffer(file, callback) {
	...
	Fn.doWhile(function loadBlockBufferTask(next) {
...
Chainful.setMethod(function addBlock(block, target_chain) {
	...
	if (target_chain == this.chain) {
		this.emit('added_block', block, target_chain);
	} else {
		this.emit('added_fork_block', block, target_chain);
	}
...
Chainful.setMethod(function requestBlocks(block_indexes, callback) {
	...
	pledge = Fn.series(function doRequest(next) {
...
Chainful.setMethod(function proposeBlocks(input_blocks, callback) {
	...
	pledge = Fn.series(function addBlocks(next) {
...
Chainful.setMethod(function proposeBlock(block, callback) {
	...
	Fn.series(function ensureBlock(next) {
		...
		// We have no blocks whatsoever?
		if (!target_chain.last_block) {

			// Good: the block we just received is the genesis block!
			if (block.index == 0) {
				...
				that.requestBlocks(request_indexes, function done(err, blocks) {
					...
					that.proposeBlocks(blocks).then(function done() {
						// Propose the initial block again!
						pledge.resolve(that.proposeBlock(block));
					}).catch(function onError(err) {
						pledge.reject(err);
					});
				});
			}
		} else if (block.isOlderThan(target_chain.last_block) || block.equals(target_chain.last_block)) {
			...
			fork.verifyProposedBlock(block, function verified(err) {
				...
				that.emit('added_fork_block', block);
 ...
 * @return   {Buffer}
 */
Chainful.setMethod(function getPublicKey(private_key) {
	...
	// Use the compressed public key to set the public key
	ecdh.setPrivateKey(private_key, type);
