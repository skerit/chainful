...
Block.setProperty(function parent() {
	...
	if (this._parent == null && this._index && this.chain) {
		this._parent = this.chain.chain[this._index];
	}
...
Block.setStatic(function fromBuffer(buffer, chain, parent, callback) {
	...
	if (block.parent == null && block.index) {
		block.parent = chain.chain[block.index - 1];
	}
...
Block.setMethod(function mine(difficulty, private_key, public_key, callback) {
	...
	if (!miner_transaction || !miner_transaction.isOurMinerTransaction(this, public_key)) {
		...
		miner_transaction = this.chain.createTransaction('miner');
...
Block.setMethod(function releaseTransactions() {
	...
	if (this.equals(this.chain.getByIndex(this.index))) {
	...
	for (i = 1; i < this.transactions.length; i++) {
		...
		for (j = this.index; j < this.chain.length; j++) {
			block = this.chain.getByIndex(i);
		...
		if (re_add_transaction) {
			this.chain.pending_transactions.push(ours);
		}
	}
});