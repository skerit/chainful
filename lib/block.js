...
Block = Fn.inherits('Informer', 'Develry.Chainful', function Block(chainful, parent) {
	...
	if (!chainful || !(chainful instanceof ChainfulNS.Chainful)) {
		throw new Error('Can not create block without Chainful instance link');
	}
...
/**
 * The age of this block as a timestamp
 *
 * @author   Jelle De Loecker   <jelle@develry.be>
 * @since    0.2.0
 * @version  0.2.0
 ...
 * @type     {Number}
 */
Block.setProperty(function age() {
...
Block.setStatic(function fromBuffer(buffer, chainful, parent, callback) {
	...
	block = new Block(chainful, parent);
...
Block.setMethod(function mine(difficulty, private_key, public_key, callback) {
	...
	if (!public_key) {
		public_key = this.chainful.getPublicKey(private_key);
	}
